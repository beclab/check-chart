...
func CheckAppCfg(chartPath string, checkAll ...bool) error {
	cfg, err := getAppConfigFromCfgFile(chartPath)
	if err != nil {
		return err
	}
	return checkAppCfg(cfg, chartPath, checkAll...)
}

func AppCfg(chartPath string, checkAll ...bool) error {
	cfg, err := getAppConfigFromCfgFile(chartPath)
	if err != nil {
		return err
	}
...
func checkAppCfg(cfg *AppConfiguration, chartPath string, checkAll ...bool) error {
	err := vd.Validate(cfg, checkAll...)
	if err != nil {
		return err
	}
	...
	return CheckResource(chartPath, cfg)
}
